...
numbercount.to_csv("2019~2023航班數量.csv")
...
#先看目前自變數是否彼此存在高相關性,考慮欄位多重共線性（Multicollinearity）問題
...
#%%test回歸方程式放入哪些欄位：原本、三個相關性最高、DEP_DELAY-->ANS: 原本都放是最好的(由R2,MSE判斷)
...
try:
    ...
    #全部欄位
    # CREATE TABLE IF NOT EXIST flight(FL_DATE date, 
    # AIRLINE VARCHAR(100), AIRLINE_DOT VARCHAR(100), AIRLINE_CODE VARCHART(5), DOT_CODE,FL_NUMBER SMALLINT, 
    # ORIGIN VARCHAR(10), ORIGIN_CITY, DEST, DEST_CITY,
    # CRS_DEP_TIME, DEP_TIME, DEP_DELAY SMALLINT, TAXI_OUT SMALLINT, WHEELS_OFF,WHEELS_ON, TAXI_IN SMALLINT, 
    # CRS_ARR_TIME, ARR_TIME, ARR_DELAY SMALLINT,
    # CANCELLED TINYINT, CANCELLATION_CODE VARCHAR(5), DIVERTED TINYINT, 
    # CRS_ELAPSED_TIME,ELAPSED_TIME, AIR_TIME INT, DISTANCE INT, 
    # DELAY_DUE_CARRIER SMALLINT,DELAY_DUE_WEATHER SMALLINT, DELAY_DUE_NAS SMALLINT, DELAY_DUE_SECURITY SMALLINT,DELAY_DUE_LATE_AIRCRAFT SMALLINT
